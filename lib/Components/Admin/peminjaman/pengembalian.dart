...
class pengembalian extends StatefulWidget {
  @override
  _pengembalian createState() => _pengembalian();
}

class _pengembalian extends State<pengembalian> {
  Response? response;
  var dio = Dio();
            ...
            children: [
              Container(
                child: ListView.builder(
                  ...
                  shrinkWrap: true,
                  itemCount: dataPengembalian == null
                      ? 0
                      : dataPengembalian.length, //penghitung item
                  itemBuilder: (BuildContext context, int index) {
                    int reverseIndex = dataPengembalian.length - 1 - index;
                    return cardPeminjaman(dataPengembalian[reverseIndex]);
                  },
                ),
              )
            ],
  ...
  void getDataPengembalian() async {
    ...
    var msg;
    try {
      response = await dio.get('$getAllPengembalian');

      status = response!.data['sukses'];
      msg = response!.data['msg']; //response untuk ambil data dari respon api
      if (status) {
        setState(() {
          dataPengembalian = response!.data['data']; //show datas
  ...
  void persetujuan(setuju, id) async {
    utilsApps.showDialog(context); //pop up message dialog

    bool status;
    var msg;
    try {
      response = await dio.put('$putPeminjaman/$id', data: {
        'persetujuan': setuju,
      });
